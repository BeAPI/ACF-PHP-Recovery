...
function acf_php_recovery_page() {
  ...
  <table class="widefat">
    ...
    <tr>
      ...
      <td><?php $matches = $wpdb->get_results("SELECT ID, post_title FROM $wpdb->posts WHERE post_title LIKE '%{$field_group['title']}%' AND post_type='".ACFPR_GROUP_POST_TYPE."' AND post_status='publish'");
          if(empty($matches)) {
            echo '<em>none</em>';
          } else {
            $links = array();
